<template>
  <div class="text-slate-100 py-1 px-4">
    <div v-if="size === 'small'" class="flex flex-col justify-center text-center pt-4">
      <div class="text-3xl font-bold text-blue-400 mb-1">{{ data.position }}</div>
    ...
    <div v-else-if="size === 'medium'" class="space-y-2">
      <div class="bg-slate-700/30 rounded-md px-3 py-2 mb-3 flex justify-between">
        <div class="text-xs text-slate-400 uppercase tracking-wider">Team</div>
        <div class="text-xs text-slate-400 uppercase tracking-wider">Pts</div>
      </div>
      <div v-for="(team, index) in data.standings.slice(0, 5)" :key="team.name" 
           class="flex justify-between items-center py-2 px-3 border-b border-slate-700/50 last:border-b-0">
        <div class="flex items-center">
          <span class="w-6 text-xs font-medium text-slate-300">
            {{ index + 1 }}
          </span>
    ...
    <div v-else class="space-y-1">
      ...
      <div v-for="(team, index) in data.standings" :key="team.name" 
        ...
        <div class="flex items-center col-span-2">
          <span class="w-6 text-xs font-medium mr-2 text-slate-300">
            {{ index + 1 }}
          </span>
